...
// intersectionWatcher contains the location of crossings detected by path.getIntersections() in the first entry
// and the boolean value of whether to cross over or under in the second value
var intersectionWatcher = [[],[]];
var previous_polynomial = [1]; // A watcher for the alexanderPolynomial
...
function showIntersections() { // This detects and draws crossings
...
function typesetInvariants() {
	...
	if (globals.isomorphy) {
			if (polyToInt(p).value != polyToInt(previous_polynomial)) {
				popUndo();
				return
			}
	}
	previous_polynomial = p;
	candidates.innerHTML = "";
	var candidateArray = knotEncyclopedia[p][0];
	if (candidateArray.length > 0) {
		for (var k = 0; k < candidateArray.length; k++) {
			var knot = candidateArray[k];
			candidates.innerHTML += "<a target='_blank' rel='noreferrer noopener' href='http://katlas.org/wiki/" + knot + "'> <img height='50px' src='images/" + knot + ".gif'> </a>";
		}
	}
